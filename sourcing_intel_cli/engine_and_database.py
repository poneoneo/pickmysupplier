...
def save_all_changes(
	...
	process, it will raise a RuntimeError with the error message.
	...
	:raises RuntimeError: If any error occur during the process of creating tables.
	"""
...
def add_suppliers_to_db(suppliers: Sequence[SupplierDict], engine_db: Engine):
	...
	Supplier model. If any error occur during the process, it will raise a RuntimeError with the
	error message.
	...
	:param engine_db: The database engine object.
	:type engine_db: sqlalchemy.engine.Engine
	:raises RuntimeError: If any error occur during the process of adding suppliers.
	"""
			...
			for supplier in suppliers:
				...
				try:
					...
					if "no such table" in str(e):
						raise RuntimeError(
							"Database tables don't exist yet. Call save_all_changes() to create them "
					...
					session.rollback()
					logger.warning(
						f"Skipping supplier '{supplier['name']}': already exists in the database ({e})"
					)
					continue
...
def add_products_to_db(products: Sequence[ProductDict], engine_db: Engine):
	...
	Product model. If any error occur during the process, it will raise a RuntimeError with the
	error message.
	...
	:param engine_db: The database engine object.
	:type engine_db: sqlalchemy.engine.Engine
	:raises RuntimeError: If any error occur during the process of adding products.
	"""
		...
		with Progress(
			...
			for product in products:
				...
				try:
					...
					session.rollback()
					logger.warning(
						f"Skipping product '{product['name']}': already exists in the database ({e})"
					)
					continue


def _related_supplier(current_session: Session, supplied_by: str) -> Supplier:
	"""
	...
	:rtype: Supplier
	:raises RuntimeError: If no supplier named `supplied_by` exists in the database.
	"""
	logger.info("Getting related supplier ...")
	query_statement = select(Supplier).where(Supplier.name == supplied_by)
	supplier = current_session.exec(query_statement).first()
	if supplier is None:
		raise RuntimeError(
			f"Data inconsistency: no supplier named '{supplied_by}' was found for a "
			"scraped product. This usually means the scraped HTML pages are incomplete "
			"or a supplier failed to parse correctly, or the data quality agent rejected "
			"that supplier while keeping the product. Re-scrape the affected keywords, "
			"or inspect the offending page before retrying `db-update`."
		)
	return supplier
